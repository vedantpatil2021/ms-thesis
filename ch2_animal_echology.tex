...
This chapter is about the opposite question: what happens when you apply those lessons proactively, before anything breaks, on hardware your previous system never ran on, orchestrated by a framework your previous system never used, in a domain your previous system never touched? The answer, documented through a summer 2025 field deployment, is that offline-first design holds. The principles generalize. The chapter that follows is the evidence.
...
This chapter addresses both gaps. The SmartWilds deployment introduces an orchestration layer that connects ground detection to autonomous aerial dispatch, operates the full pipeline without any external network connectivity, and does so on field-constrained ARM64 hardware using Docker Compose as the orchestration framework.
...
\begin{table}
\centering
\caption{All eight services, their ports, ownership, and roles}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{l l l X}
\hline
\textbf{Service} & \textbf{Port} & \textbf{Owner} & \textbf{Role} \\
\hline

MQTT Broker (Mosquitto)
& 1883
& Infrastructure
& Local pub/sub event routing \\
\hline

MQTT Subscriber
& --
& External
& Routes detection events; resolves topic to GPS \\
\hline

SmartFields
& 2188
& This work
& Pipeline orchestrator; three-stage mission control \\
\hline

OpenPassLite
& 2177
& This work
& Drone mission framework; flight control \\
\hline

WildWings
& 2199
& External \cite{kline2025wildwing}
& AI-guided aerial tracking \\
\hline
...
SmartFields monitors mission progress by polling service log files every two seconds for completion signatures. Success is detected by the pattern \texttt{Mission \{name\} thread finished}. Failure is detected by any of \texttt{Mission failed}, \texttt{AssertionError}, or \texttt{connection timed out}. This approach avoids tight service coupling while maintaining reliable stage transition logic.

\subsection{OpenPassLite}
\label{subsec:ch4_openpasslite_additions}
OpenPassLite is a modular drone mission framework built on the Parrot Olympe SDK \cite{parrot2024olympe}. Its defining architectural feature is a drop-in mission model: each flight behavior is a single self-contained Python file at \textit{mission/\{mission\_name\}/script.py}. New mission types require no changes to the framework core.
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/smartfield.jpg}
    \caption{Smartfield's Field Setup at Tuttle Park}
    \label{fig:fieldvalidation}
\end{figure}

Over the 20-hour deployment, the system processed 45 detection events in total. Of these, 41 resulted in a dispatch attempt; the remaining 4 events arrived while a mission was already in progress and were correctly rejected by the concurrency lock. Of the 41 attempts, 38 completed as full autonomous mission cycles, yielding a mission success rate of 92.7\%. No human intervention was required at the dispatch decision point for any mission. These figures are derived from post-deployment log file review and are reported here as field estimates consistent with direct observation during the deployment.

%% P2.4 FIX: Add proxy ecological validity caveat in results section
It is important to interpret this figure in the context of the validation conditions. The proxy human subject moved predictably within a bounded area, whereas ungulate behavior in a 220-acre enclosure involves variable movement, vegetation occlusion, GPS multipath, and detection confidence that fluctuates with animal posture and group density. The 92.7\% success rate reflects system performance under controlled proxy conditions. Performance at a live conservation site may differ, as discussed further in the limitations section of Chapter 5.
