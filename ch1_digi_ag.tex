...
\small
\cmark~=~full support;
\caption{Far-edge deployment capability comparison.}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/inflection1.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/inflection2.png}
    \caption{Inflection Point 2 resolution: a Smart Pod Scheduler coordinates
adjustable resource allocation and Pod Disruption Budgets to contain startup
resource contention on constrained single-node far-edge hardware, enabling
autonomous pod network operation.}
    \label{fig:placeholder}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{images/inflection3.png}
...
under complete network disconnection, with the Helm Resource Manager
coordinating on-demand deployment without any external dependencies.}
    \label{fig:placeholder}
\end{figure}
...
\textbf{The solution: air-gapped image pre-population with local Helm repositories.} This solution operates at the configuration level, not the source level, and builds on a documented practice in the K3S and containerd communities known as air-gapped deployment.
...
The air-gapped deployment pattern itself is not new; the K3S documentation describes it for environments without internet access. The contribution of this work is recognizing that air-gapped pre-population is a necessary and non-negotiable component of any offline-first far-edge K3S deployment, and integrating it into a systematic offline-first deployment pattern alongside the VXLAN overlay and startup sequencing solutions.
...
We further propose an inflection-point database as a community artifact: a structured, shared repository where far-edge AI practitioners can document deployment failure modes, their trigger conditions, and their resolutions. Each entry follows the structure of Table~\ref{tab:inflection_taxonomy}. The goal is to reduce the rediscovery cost that every team currently bears when they carry a lab-validated system into a new physical environment. This database does not yet exist as a community resource. We describe it here as a proposed contribution and invite the research community to build toward it.
...
The architectural solutions validated at Stoutsville are not configurations written for one farm. They are a deployment pattern: a set of platform-level decisions that, taken together, make a K3S-based far-edge AI application resilient to the three most common categories of infrastructure failure in physical field environments. The VXLAN overlay works on any site with any firewall policy. The init container startup sequencing works on any constrained single-node hardware. The air-gapped image pre-population works anywhere with zero connectivity. None of these solutions is specific to digital agriculture.
