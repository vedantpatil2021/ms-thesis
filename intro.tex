...
The dominant response in the research community has been to optimize the AI itself for constrained hardware: smaller models, quantized weights, pruned architectures\cite{deng2020compression, howard2017mobilenets}. These are genuine contributions. But they address a different failure mode. A model that runs efficiently on a microcontroller is still useless if the container it lives in cannot start because the orchestrator expected a stable network interface that the deployment site does not have. Efficiency research \cite{shi2016edge, satyanarayanan2017emergence} assumes the platform is working. In far-edge environments, that assumption needs to be earned, not inherited.
...
%% ============================================================
\textbf{Inflection-point classification framework.}
We introduce the inflection point as a formal concept in far-edge deployment analysis: a site-induced, platform-level failure mode that is separate from application bugs and requires an architectural response. Three inflection points from the OpenPASS agricultural deployment are documented, each with its triggering condition, failure mode, and architectural resolution described.

\textbf{Offline-first K3S architecture.}
We designed and implemented an offline-first deployment architecture for far-edge K3S applications, with three components that each address one of the inflection points. A VXLAN overlay anchored to a virtual dummy0 interface decouples pod networking from physical network interfaces, so the cluster initializes correctly regardless of which interface is present at the deployment site. Init container sequencing with explicit resource requests prevents crash-at-init failures on constrained single-node edge hardware; Pod Disruption Budgets accompany this to keep already-running inference pods from being voluntarily evicted during active missions. Air-gapped image pre-population via \texttt{ctr image import} with on-node Helm repositories allows the full application stack to be reconstructed without any external registry access. The architecture was deployed and validated at a real agricultural field site.
...
\textbf{Proposed inflection-point database.}
...
Chapter 4 presents the Smart Backpack wildlife monitoring system. It describes the SmartWilds project context, the eight-container Docker Compose architecture, the design of OpenPassLite and SmartFields, and the field validation results from Tuttle Park.

Chapter 5 draws both chapters into a unified analysis. It traces how the design principles developed in Chapter 3 appear in the architecture of Chapter 4, examines what that cross-domain consistency implies, and presents the proposed inflection-point database. It also covers contributions, discusses limitations, and identifies open problems.