...
\begin{table}[htbp]
  \centering
  \small
  \caption[Far-edge deployment capability comparison across container orchestration platforms.]{%
    Far-edge deployment capability comparison across container orchestration platforms
    evaluated for disconnected field operation. OpenPASS (this work) extends K3s (vanilla)
    with targeted architectural solutions for all three infrastructure gaps identified at
    Stoutsville, Ohio.\\[4pt]
    \cmark~=~full support;\quad
    \pmark~=~partial support;\quad
    \xmark~=~not supported;\quad
    \namark~=~not applicable / out of scope for that platform's design target.%
  }
