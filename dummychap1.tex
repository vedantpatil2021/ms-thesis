...
\centering
\small
\caption{Far-edge deployment capability comparison across container orchestration platforms evaluated for disconnected field operation. OpenPASS (this work) extends K3s (vanilla) with targeted architectural solutions for all three infrastructure gaps identified at Stoutsville, Ohio. \textit{Legend:} \cmark~=~full support; \pmark~=~partial support; \xmark~=~not supported; \namark~=~not applicable / out of scope for that platform's design target.}
