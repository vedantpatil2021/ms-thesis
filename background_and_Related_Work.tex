...
%% ============================================================
The dominant research thread in edge AI has focused on reducing the computational footprint of AI models to fit within the resource envelope of edge devices. Quantization reduces weight precision from 32-bit or 16-bit floating point to 8-bit or lower integer representations, substantially reducing both model size and inference latency \cite{jacob2018quantization, 
wang2023ondevice}. Pruning removes redundant parameters. Knowledge distillation trains smaller student models to replicate the behavior of larger teacher models\cite{hinton2015distilling}. These techniques have enabled deployment of sophisticated computer vision models on devices ranging from microcontrollers to single-board computers\cite{deng2020compression}.

YOLO-family models have become the standard for real-time object detection in edge AI applications \cite{redmon2016yolo}. YOLOv5 in particular has been widely adopted for wildlife detection in drone imagery, achieving strong detection performance on aerial datasets while remaining deployable on GPU-equipped field laptops and edge hardware \cite{waid2023dataset}. The progression from YOLOv5 through YOLOv8 and YOLOv11 has continued to improve accuracy-efficiency trade-offs relevant to edge deployment scenarios \cite{jocher2020yolov5, waid2023dataset}. Energy-efficient object detection specifically for UAV platforms has been demonstrated by Suo et al., whose E\textsuperscript{3}-UAV system achieves real-time inference on drone hardware within the power constraints of battery-operated aerial platforms, directly targeting the resource envelope of far-edge aerial deployments \cite{suo20233}.
%% ============================================================
...
%% ============================================================
Precision agriculture uses fine-grained spatial and temporal data to optimize farm management decisions. Drones equipped with multispectral, hyperspectral, and thermal cameras have emerged as central tools for crop monitoring, enabling field-scale surveys of crop health, pest damage, nutrient deficiency, and growth patterns at resolutions that satellite imagery cannot match and at frequencies that ground surveys cannot sustain \cite{zhang2012uas, habib2024transforming}. Anam et al. provide a systematic review of UAV and AI integration in precision agriculture, documenting the range of detection targets and algorithmic approaches deployed across crop types and geographic regions \cite{anam2024uav}. Reinforcement learning approaches have extended drone-based agriculture from passive sensing toward fully autonomous field coverage: Zhang et al. demonstrate whole-field aerial scouting without human intervention, using a deep RL agent that plans and executes survey missions end-to-end across large agricultural areas \cite{zhang2020whole}. Boubin et al. develop MARBLE, a multi-agent reinforcement learning system that coordinates autonomous sensing across field-scale agricultural environments directly at the edge, without cloud coordination \cite{boubin2022marble}. AI-driven validation of digital agriculture models has confirmed that such systems can achieve accuracy competitive with expert annotation across multiple crop stress categories \cite{romero2023ai}. The sustainability and scalability implications of AI-based agricultural monitoring are surveyed broadly by Mana et al., who examine production agriculture AI deployments across crop types and management contexts \cite{mana2024sustainable}.
...
The TELUS Agriculture industry pilot cited in conference presentations as an example of cloud-native AI deployment across the full edge-to-cloud continuum validates the architecture when connectivity is present in \cite{gu2025cloudedge}. It does not address the case where connectivity is absent. The gap between what precision agriculture needs (fast, on-site analysis) and what the existing deployment literature covers (connected edge node coordinating with cloud) is where OpenPASS operates. Boubin et al. characterize the resource management challenge specific to fully autonomous aerial systems at the edge, demonstrating that co-locating AI inference with flight control on constrained edge hardware produces scheduling and resource contention problems not addressed by standard orchestration platforms \cite{boubin2019managing}. Barbedo et al. examine explainability and privacy constraints in AI-enabled soybean crop monitoring, identifying system-level deployment requirements that further restrict the architectural options available to field-deployed inference pipelines \cite{barbedo2026explainability}.
...
Manual drone piloting for behavioral observation creates an operational bottleneck that constrains data quality and throughput. An operator who must simultaneously control the aircraft and operate the camera cannot maintain consistent observation geometry, altitude, or tracking behavior across missions. WildWings, developed by Kline et al. and published in Methods in Ecology and Evolution, addresses this limitation with an autonomous drone system that uses YOLOv5su detection and a herd-centroid navigation policy to track group-living animals without pilot input \cite{kline2025wildwing}. The system achieves 87\% navigation accuracy compared to an expert pilot in simulation, and produces high-quality annotated aerial video suitable for downstream behavioral analysis. WildWings represents the state of the art in autonomous aerial wildlife tracking. Kline et al. examine collective animal behavior monitoring with drones and computer vision at scale, characterizing the observation geometry, temporal resolution, and tracking continuity requirements for behavioral ecology research applications \cite{kline2025studying}. A companion open-source implementation described in \cite{kline2026wildwing} provides the engineering documentation that makes the WildWing platform accessible to ecological research groups without specialized drone engineering expertise.
%% ============================================================
\subsection{The Closed-Loop Gap}
\label{sec:closed_loop_gap}
%% ============================================================
What neither camera trap AI systems nor autonomous aerial tracking systems address is the pipeline that connects them. Camera trap networks detect events and log them. Autonomous aerial systems can track animals once positioned over them. The handoff between the two — the decision to dispatch the drone, navigate it to the detection site, and initiate tracking — remains a human task in every documented system. No prior work constructs a fully autonomous pipeline from ground-level detection event to aerial behavioral observation without human intervention at the dispatch decision point. Kline et al. propose a framework for autonomic computing for in situ imageomics that addresses multi-modal coordination of sensing assets in ecological field deployments, providing architectural grounding for integrating camera and aerial observation layers \cite{kline2023framework}. Grushchak et al. address decentralized multi-drone coordination for wildlife video acquisition, demonstrating cooperative aerial coverage of large enclosures, but the triggering mechanism for drone dispatch remains human-mediated rather than camera-trap-driven \cite{grushchak2024decentralized}.
